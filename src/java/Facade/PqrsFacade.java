...
public class PqrsFacade extends AbstractFacade<Pqrs> {
    ...
    public List<Pqrs> consultarCategoria(String cat){
        Query q = em.createQuery("SELECT p FROM Pqrs p WHERE p.estado = 1 AND p.tipo=:cat");
        q.setParameter("cat", cat);
        return q.getResultList();
    }
    
    public List<Pqrs> consultarAcudiente(int id){
        Query q = em.createQuery("SELECT p FROM Pqrs p WHERE p.estado = 1 AND p.idAcudiente.idUsuario =:id");
